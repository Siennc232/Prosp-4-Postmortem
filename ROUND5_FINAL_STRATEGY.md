...
The final source code was lost and cannot be uploaded.
...
The evidence was clear in the live log. It contained 131 submission fills, or about 5 fills per 1,000 ticks at the median product level. The old BBO-cross replay produced about 1. The trade-event replay produced about 120 fills. It was not perfect, but it was much closer to the real execution process.

I also found a PEBBLES order-limit bug. The base strategy and the overlay could each use the full capacity. This caused eight live rejections: five in `PEBBLES_XL` and three in `PEBBLES_XS`. I fixed this with per-tick order reservation and checked that the simulated reject count was zero.
