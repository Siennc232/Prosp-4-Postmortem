...
Inspired by the [2nd team](https://github.com/TimoDiehm/imc-prosperity-3) in Prosperity 3. The best bid and best ask can move because of very small orders. This made the raw mid-price noisy in the Round 3 and Round 4 data. I therefore compared it with a `wall-mid`: the average of price levels that held meaningful displayed depth.
...
This choice was concrete. The [decision audit](data/r4_balanced_vs_maxraw_acceptance.csv) records 201,435.5 over three days for the maximum-return reference and 198,915.5 for the balanced version, a difference of 2,520. Under the stress +200 test, the balanced version produced 43,715.5 versus 34,235.5. I accepted the smaller headline result because the stress result improved by 9,480.
...
The third figure compares the final balanced version with the maximum-return reference using the [decision audit](data/r4_balanced_vs_maxraw_acceptance.csv) and [per-day replay metrics](data/r4_balanced_vs_maxraw_metrics.csv). It is the source for the Round 4 return-versus-stress comparison above.

![Round 4 balanced versus maximum-return comparison](assets/research/r4_balanced_vs_maxraw.png)

Round 5 is intentionally absent from the notebook because its final source code was lost and its raw event-backtest CSVs are not available.
